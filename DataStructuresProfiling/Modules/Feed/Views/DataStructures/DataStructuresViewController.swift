...
class DataStructuresViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    lazy var parentContext: NSManagedObjectContext = {
        guard let appDelegate = UIApplication.shared.delegate as? AppDelegate,
            let coreDataStack = appDelegate.coreDataStack else {
            fatalError()
        }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(barButtonSystemItem: .cancel, target: self, action: #selector(closeTouchUpInside))
        testOnlyButton.isEnabled = false
        resultsTableView.isScrollEnabled = false
        updateCountLabel()
    }
    
    //MARK: Create/Test/Save
